...
export default function CartMenu({ showcart, setShowcart }) {
  const { products, shopcart, totalvalue, setTotalvalue } =
  ...
  useEffect(() => {
    if (list.length) {
      setTotalvalue(
        list.map((product) => product.price).reduce((a, b) => a + b)
      );
    } else {
      setTotalvalue(0);
    }
  ...
  if (showcart) {
    return (
      <CartMenuLayer>
        <div onClick={() => setShowcart(false)}></div>
        <menu>
          ...
          {!user ? (
            ...
            <button
              onClick={() =>
                navigate("/purchase-confirmation", { state: list })
              }
            >
              Finalizar Compra
            </button>
